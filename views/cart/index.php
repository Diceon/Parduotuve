...
<?php } ?>
<?php } ?>
<?php if(isset($this->data['products']) && is_array($this->data['products'])) { ?>
    <h1 class="text-center mt-3">Cart</h1>
        <ul class="list-group">
<?php foreach ($this->data['products'] as $key => $product) { ?>
            <li class="list-group-item"><a href="/parduotuve/product/<?php echo $product['id']; ?>"><?php echo $product['name']; ?></a> - $<?php echo $product['price']; ?> x <?php echo $product['amount']; ?> <form action="/parduotuve/cart" method="post" class="d-inline   "><button type="submit" class="close" name="cart_remove" value="<?php echo $product['id']; ?>"><span class="text-danger" aria-hidden="true">&times;</span></button></form></li>
<?php } ?>
        </ul>
        ...
        <a href="/parduotuve/cart/" class="float-right"><button class="btn btn-primary">Continue</button></a>
<?php } else {?>
        <div class="my-5">
